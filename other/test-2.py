...
class EngramModule(nn.Module):
    ...
    def update_engram_memory_ema(self, embedding: nn.Embedding, x_features, target_ids, momentum=0.99):
        with torch.no_grad():
            current_memory = embedding.weight[target_ids]
            updated_memory = (momentum * current_memory) + ((1.0 - momentum) * x_features.mean(dim=1))
            embedding.weight[target_ids] = updated_memory
